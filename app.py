...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .date-input-wrap input[type="month"]:focus {
      border-color: #6b7aab;
    }
  ...
  <div class="card">
    ...
    <div class="checks-body" id="checks-body">
      <ol>
        ...
        <li><strong>Duplicate Bills</strong> — Bills where the same Company + Bill # appears more than once in QBO</li>
    ...
    <form id="recon-form" enctype="multipart/form-data">
      ...
      <div class="field-group">
        <label class="field-label">QBO Export(s)</label>
      ...
      <!-- Month-end date -->
      <div class="field-group">
        <label class="field-label" for="period-input">Month End Date</label>
        <div class="date-input-wrap">
          <input type="month" id="period-input" name="period" placeholder="Select a month">
          <svg class="cal-icon" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
...
def run():
    try:
        ...
        period_input = request.form.get('period', '').strip()
        if period_input:
            try:
                from datetime import datetime as dt
                period_label = dt.strptime(period_input, '%Y-%m').strftime('%B %Y')
            except ValueError:
                period_label = period_input
        else:
