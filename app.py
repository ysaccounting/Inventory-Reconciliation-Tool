...
<body>
    ...
    <form id="recon-form" enctype="multipart/form-data">
      ...
      <div class="field-group">
        ...
        <div class="upload-zone" id="dm-zone">
          ...
          <div class="zone-hint">
            Drag &amp; drop your <strong>Excel or CSV</strong> file here, or <a href="#">browse</a>
          </div>
          <div class="file-list" id="dm-file-list"></div>
        </div>
      </div>
  ...
  <script>
    ...
    dmInput.addEventListener('change',  () => renderFileList(dmInput,  document.getElementById('dm-zone'),  document.getElementById('dm-file-list')));
    ...
    clearBtn.addEventListener('click', () => {
      // Reset file inputs
      [qboInput, pdInput, ccInput, dmInput].forEach(input => {
        const dt = new DataTransfer();
        input.files = dt.files;
      });
      ...
      renderFileList(dmInput,  document.getElementById('dm-zone'),  document.getElementById('dm-file-list'));
...
def run():
    try:
        ...
        for f in dm_files:
            if f.filename == '': continue
            raw = f.read()
            dm_input_files.append((f.filename, raw))
            buf = BytesIO(raw)
            buf.name = f.filename
            dm_buffers.append(buf)
        ...
        report_bytes = build_report(
            ...
            dm_buffers=dm_buffers if dm_buffers else None
        )
